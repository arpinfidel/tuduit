...
func (a *App) CreateSchedule(ctx *ctxx.Context, p CreateScheduleParams) (next string, err error) {
	userID := ctx.User.ID
	if p.Assignee != "" {
	...
	s, err := a.d.ScheduleUC.Create(ctx, nil, []entity.Schedule{schedule})
	if err != nil {
		return "", err
	}
	sched := s[0]
	...
	_, err = a.d.TaskUC.Create(ctx, nil, []entity.Task{
		template,
		task,
	})
	if err != nil {
